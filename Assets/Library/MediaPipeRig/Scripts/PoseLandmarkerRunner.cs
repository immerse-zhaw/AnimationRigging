...
namespace Library.MediaPipeRig.PoseLandmarkDetection
{
    public class PoseLandmarkerRunner : VisionTaskApiRunner<PoseLandmarker>
    {
        ...
        [SerializeField] private BodyTrackerRig humanoidRig;
        ...
        private void OnPoseLandmarkDetectionOutput(PoseLandmarkerResult result, Mediapipe.Image image, long timestamp)
        {
            lock (_resultLock)
            {
                _latestResult = result; // Struct assignment (copy)
                _hasNewResult = true;
            }
        ...
        private void Update()
        {
            if (_hasNewResult)
            {
                PoseLandmarkerResult resultCopy;

                lock (_resultLock)
                {
                    resultCopy = _latestResult;
                    _hasNewResult = false;
                }

                if (resultCopy.poseWorldLandmarks?.Count > 0 && humanoidRig != null)
                {
                    humanoidRig.ApplyPose(resultCopy);
                }
            }
        }
    }
